...
from certlm.language_models import CHATGPT, get_language_model
from certlm.utils.util import LMInput

logger = logging.getLogger(__name__)
class MCQAnswerSampler(BaseSampler):
    """
    ...
    def _predict(self, question_dict):
        ...
        """
        full_question = question_dict['lm_input']['message'].strip().split('\n\n')
        question = full_question[0]
        full_choice = full_question[-1].replace('Choices: ', '').split(',')
        choices = [f"  - {item}" for item in full_choice]
        choices = '\n'.join(choices)
