...
def load_relation(relation_file, relation_name):
    ...
    for sample in data:
        ...
        object_label = sample["obj_labels"]  # Y
        subject_uri = sample["sub_uri"]

        object_uris = sample["obj_uris"]
...
def eval_short_answer_question(data_dir, fout, fdetail, relation_dir, unknown_responses, template='single_answer'):
    ...
    for pred_file in all_outputs:
        ...
        relation_file = f"{args.relation_dir}/{relation_name}/all.jsonl"
        ...
        with open(pred_file) as fin, open(eval_file, 'w') as feval:
            for line in fin:
                ...
                if not is_correct:
                    for correct_ans in record['obj_aliases']:
                        for alias in correct_ans:
                            is_correct = is_correct or fuzzy_match(alias, predict)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, help='Data dir',
                        default='/home/tvuu0014/workspace/certfiedlm/eval-result/wiki_bio/questions_template/chatgpt')
    ...
    parser.add_argument('--relation-dir', type=str, help='Relation dir',
                        default='/home/tvuu0014/workspace/certfiedlm/chatgptEval/kg_examples/wiki-bio/triples_processed')
